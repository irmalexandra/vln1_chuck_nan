class UIEmployees():
    ...
    def display_selected_model_menu(self, model):
        if model.get_title() == "Pilot":
            nav_dict = {1: self.display_edit_employee_menu,
                        2: self.display_work_schedule,
                        3: self.change_pilot_licence,
                        9: self.__ui_base_functions.back,
                        0: self.__ui_base_functions.home}
            employee_menu = "1. Edit Employee 2. Work Schedule 3. Change Licence"
            return_value = self.__ui_base_functions.display_menu(
                employee_menu, nav_dict, model)
            return self.__ui_base_functions.check_return_value(return_value)
        else:
            nav_dict = {1: self.display_edit_employee_menu,
                        2: self.display_work_schedule,
    ...
    def change_pilot_licence(self, model):
